...
SRCVER=qemu-1.5.2
PKG=opt-x86_64-$SRCVER-2 # with build version
...
sed -i 's/"elf.h"/"xxelf.h"/' hw/i386/multiboot.c

#########
# Configure
OPTPREFIX=opt/qemu
B-configure-2 --prefix=/$OPTPREFIX --localstatedir=/var --target-list=arm-softmmu,x86_64-softmmu --enable-kvm --disable-xen --audio-drv-list= --static || exit 1
