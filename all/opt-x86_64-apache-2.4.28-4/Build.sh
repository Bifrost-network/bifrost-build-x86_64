...
PKG=opt-x86_64-apache-$APACHEVER-4 # with build version
...
pkg_install musl-libmd-1.0.0-1 || exit 2
...
patch -p0 < $PKGDIR/mod_ssl_md-2.4.x-v5.diff || exit 1
...
cp $PKGDIR/mod_md.c modules/http || exit 1
cp $PKGDIR/mod_md.h modules/http || exit 1
cp $PKGDIR/mod_md_config.h modules/http || exit 1
cp $PKGDIR/mod_md_os.h modules/http || exit 1
cp $PKGDIR/mod_md_private.h modules/http || exit 1

patch -p1 < $PKGDIR/httpd-debug.pat || exit 1

# Backported from 2.4.29 or later
patch -p3 < $PKGDIR/backport1.pat || exit 1
...
LIBS="-lmd -lcurl -ljansson" B-configure-1 --prefix=/opt/apache --disable-nls --enable-static-support\
 ...
 --with-module=aaa:limitipconn,proxy:lbmethod_byip,http:md \
